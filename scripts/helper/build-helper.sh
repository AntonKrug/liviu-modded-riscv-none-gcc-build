...
# Multi-platform helper for RISC-V tools, OpenOCD and QEMU builds, using Docker.
...
do_host_build_target() {
  ...
  if [ "${docker_avoid}" == "y" ]
  then
    # ignore the docker image supplied and run the build natively, not the same
    # as native build which has the same target as is the host, here it's still
    # possible to do win builds on a debian host without a docker
    echo "Avoiding docker build, and running commands directly on the host"
    echo $docker_avoid
    docker_image=""
  fi
...
do_container_create_distribution() {
  ...
  if [ "$#" == "1" -a "$1" == "--draft" ]
  then
    #with --draft create only archives, but not the installers
    DRAFT="y"
  fi
  ...
  mkdir -p "$(dirname "${output_folder_path}")"
      ...
      then
        ...
        then
          ...
          echo
          echo "Skipping setup/installer generator becuase of --draft"
          echo
        fi
      ...
      then
        ...
        tar -c -z -f "${distribution_archive}" --owner root --group root gnu-mcu-eclipse
...
do_container_completed() {
      ...
      then
        ...
        then
          ...
          echo "Archive file ${distribution_archive} created."
        fi
